...
public record struct TieMetadata : ILunaObject, ILunaSerializable
{
    ...
    public TieMetadata(LunaStream stream, bool isOld, uint index = 0)
    {
    ...
    public byte[] ToBytes(bool isOld, params object[]? additionalParams)
    {
        ...
        {
            throw new InvalidOperationException($"[WONKY_CONVERT_ERR] Data have been lost while turning a {nameof(TieMetadata)} into an array of bytes: Sizes does not match (0x{rented.Length:X}/0x{Size:X})");
        }
        return rented;
    }
}
