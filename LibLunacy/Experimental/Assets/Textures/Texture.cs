...
namespace LibLunacy.Experimental.Assets.Textures;

/// <summary>
/// Concrete texture implementation
/// </summary>
public sealed class Texture : ITexture
{
    ...
    public byte[] GetMipmapData(int level)
    {
        ...
        // Get full data and calculate mipmap offset
        var fullData = GetPixelData();

        if (level == 0)
            return fullData;

        // Calculate offset to requested mipmap level
        int offset = 0;
    ...
    /// </summary>
    public static Texture FromData(
        ulong id,
        uint width,
        uint height,
        TextureFormat format,
