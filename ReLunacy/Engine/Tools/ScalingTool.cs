...
public class ScalingTool : BasicTransformTool
{
    ...
    {
        if(entity.EntityType == EntityType.Moby || entity.EntityType == EntityType.UFrag)
        {
            float dist = (pivot - data.cameraPos).Length;

            Vec3 mousePrevPos = data.cameraPos + dist * data.mousePrevDir.Normalized();
            Vec3 mouseCurrPos = data.cameraPos + dist * data.mouseCurrDir.Normalized();

            float prevDist = (pivot - data.cameraPos).Length;
            float currDist = (pivot - data.cameraPos).Length;
