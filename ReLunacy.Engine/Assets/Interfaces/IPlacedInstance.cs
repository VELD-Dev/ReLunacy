...
public interface IPlacedInstance<out TAsset> where TAsset : IAsset
{
    ...
    /// <summary>Distance (in-game units) beyond which the game stops updating this instance's logic (a
    /// separate budget from <see cref="DisplayDistance"/>, which only gates rendering). &lt; 0 means
    /// unlimited. Only Mobys carry this from the file; other instance types default to unlimited. Not
    /// used by anything in ReLunacy's own rendering/culling - carried purely so it's visible and
    /// editable in the Property Inspector, the way ReLunacy-Ymir exposes it.</summary>
